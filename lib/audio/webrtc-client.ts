...
export interface StreamConfig {
  type: "flue" | "direct"
  key?: string
  secret?: string // Added secret for WHIP authentication
...
export class WebRTCClient {
  ...
  async publishToFlueStream(config: StreamConfig, localStream: MediaStream): Promise<void> {
    ...
    try {
      ...
      if (config.type === "flue" && config.key && config.secret) {
        const whipUrl = `https://app.flue.live/live/v1/whip?app=live&stream=${config.key}&secret=${config.secret}`
        ...
        if (!response.ok) {
          throw new Error(`WHIP connection failed: ${response.status} - ${await response.text()}`)
        }
        ...
        throw new Error("Missing required config: key and secret are required for Flue.live WHIP")
      }
    } catch (error) {
